...
class TestCity(unittest.TestCase):
    ...
    def test_state_relationship_deletes(self):
        """
        Test for the  delete cascade in City-State relationship
        """
        st = State(name="California")
        self.dbstorage._DBStorage__session.add(st)
        self.dbstorage._DBStorage__session.commit()
        ct = City(name="Malibu", state_id=st.id)
        ...
        db = MySQLdb.connect(user="hbnb_test",
                             ...
                             db="hbnb_test_db")
        cursor = db.cursor()
        cursor.execute("SELECT * FROM cities WHERE BINARY name = 'Malibu'")
    ...
    def test_is_subclass(self):
        """
        Test for Checking that City is a subclass of BaseModel
        """
    ...
    def test_two_models_are_unique(self):
        """
        Test for different City instances are unique
        """
        ct = City()
